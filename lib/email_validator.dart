// Features
// 1. Email Validation: Provides a validate function to check if an email address is valid based on RFC standards.
// Supports local-parts with dots or quoted strings.
// Validates domains with subdomains, top-level domains (optional), and IPv4/IPv6 addresses (in brackets).
// 2. Flexibility: Allows enabling/disabling top-level domains and international email standards using optional parameters in validate.
// 3. String Handling: Includes functions for checking character types (letters, digits, atoms), skipping quoted text, and parsing IPv4/IPv6 literals.
// 4. Domain Validation: Checks subdomains, top-level domains, and domain types (alphabetic, numeric, alphanumeric).
// Handles international domains using allowInternational option.
// 5. Error Handling: Returns false for invalid emails, indicating specific validation failures.
// 6. Structure: Uses private helper functions for modularity and readability.
// 7. No Regular Expressions: Validates emails using custom logic for potentially better performance and flexibility.
// 8. Customizable: Offers options to control validation behavior (top-level domains, international standards).
// 9. Maintainability:Uses clear naming conventions and comments for better understanding.
// 10. Potential Enhancements: Consider adding support for punycode to handle international domains more robustly.
